...
class FilePlayer extends import_react.Component {
  ...
  shouldUseHLS(url) {
    ...
    return import_patterns.HLS_EXTENSIONS.test(url);
  }
  shouldUseDASH(url) {
  ...
  load(url) {
    ...
    if (this.shouldUseHLS(url)) {
      this.hls = new import_hls.default(hlsOptions);
      this.hls.on(import_hls.default.Events.MANIFEST_PARSED, () => {
        this.props.onReady();
      });
      this.hls.on(import_hls.default.Events.ERROR, (e, data) => {
        this.props.onError(e, data, this.hls, import_hls.default);
      });
